...
class GraphDFSApp:
    ...
    def _create_ui(self):
        ...
        search_cfg_box = ttk.LabelFrame(left_panel, text="1. Параметри пошуку DFS", padding=6)
        ...
        ttk.Label(search_cfg_box, text="Мета пошуку:").pack(anchor=tk.W, pady=(4, 0))
        self.mode_var = tk.StringVar(value="shortest")
        ttk.Radiobutton(search_cfg_box, text="Найкоротший шлях (DFS B&B)", variable=self.mode_var, value="shortest").pack(anchor=tk.W)
        ttk.Radiobutton(search_cfg_box, text="Перший знайдений шлях (DFS)", variable=self.mode_var, value="first").pack(anchor=tk.W)
    ...
    def load_chaotic_graph(self):
        """Генерує складнозв'язний хаотичний граф: порядок 32, розмір 44, наявні цикли та дуги"""
        self.reset_search_state()
        self.nodes.clear()
        self.edges.clear()

        # 1. Зафіксовані координати 32 вершин у вигляді трьох нерівномірних зон (кластерів)
        coords = {
            # Центральне ядро (цикл із діагоналями)
            1: (450, 270), 2: (400, 210), 3: (500, 210), 4: (400, 330), 5: (500, 330),
            # Західно-північний кластер
            6: (280, 150), 7: (190, 110), 8: (120, 180), 9: (200, 230), 10: (290, 270),
            11: (150, 310), 12: (80, 260), 13: (90, 390), 14: (180, 420),
            # Східно-північний кластер
            15: (620, 150), 16: (710, 110), 17: (780, 180), 18: (700, 230), 19: (610, 270),
            20: (750, 310), 21: (820, 260), 22: (810, 390), 23: (720, 420),
            # Південна обхідна мережа (поперечні та периметральні зв'язки)
        ...
        # 2. 44 ребра (38 ненапрямлених ребер + 6 напрямлених дуг для перевірки орієнтованості)
        raw_edges = [
            # Центральний блок
            (1, 2, False), (1, 3, False), (1, 4, False), (1, 5, False),
            (2, 3, False), (4, 5, False), (2, 4, False), (3, 5, False),
            # Зв'язки центру з лівим крилом
            (2, 6, False), (4, 10, False), (1, 10, False),
            # Ліве крило (цикли та перетини)
            ...
            # Зв'язки центру з правим крилом
            (3, 15, False), (5, 19, False), (1, 19, False),
            # Праве крило (цикли та дуги)
            ...
            # Північна та південна перемички
            ...
            # Напрямлені дуги (орієнтовані переходи між кластерами)
            (10, 19, True),   # Дуга: лівий кластер -> правий кластер
            (24, 11, True),   # Дуга: низ -> середина лівого крила
            (23, 28, True),   # Дуга: праве крило -> низ
            (29, 1, True),    # Дуга: північ -> центр
            (25, 32, True),   # Дуга: низ діагональ
            (27, 32, True)    # Дуга: низ діагональ
        ]
    ...
    def start_auto_search(self):
        if not self.is_running:
            ...
            self.search_generator = self.dfs_generator()
    ...
    def redraw_graph(self, active_node=None, current_path=None, final_path=None, is_best=False, pruned=False):
        ...
        tree_edges = {
            (1, 2), (1, 3), (1, 4), (1, 5), (1, 6),
            (2, 7), (2, 8), (3, 9), (3, 10), (4, 11), (4, 12), (5, 13), (5, 14), (6, 15), (6, 16),
            (7, 17), (7, 18), (8, 19), (9, 20), (9, 21), (10, 22), (11, 23), (11, 24),
            (12, 25), (13, 26), (13, 27), (14, 28), (15, 29), (15, 30), (16, 31)
        }
        ...
        for (u, v), meta in self.edges.items():
            ...
            is_cross_edge = (min(u, v), max(u, v)) not in tree_edges
    ...
    def gui_toggle_edge_type_dialog(self):
        if not self.edges:
            return
        dialog = tk.Toplevel(self.root)
        dialog.title("Зміна ребро ↔ дуга")
        dialog.geometry("320x150")
        dialog.transient(self.root)

        edge_strs = [f"{u} {'->' if m['directed'] else '<->'} {v} (зараз: {'дуга' if m['directed'] else 'ребро'})" for (u, v), m in self.edges.items()]
        edge_keys = list(self.edges.keys())

        ttk.Label(dialog, text="Оберіть зв'язок для перетворення типу:").pack(pady=4)
        cb = ttk.Combobox(dialog, values=edge_strs, state="readonly")
        cb.pack(fill=tk.X, padx=10, pady=4)
        cb.current(0)
        ...
        def toggle():
            ...
            u, v = old_key
            old_directed = self.edges[old_key]['directed']
            del self.edges[old_key]

            new_directed = not old_directed
            new_key = (u, v) if new_directed else (min(u, v), max(u, v))
            self.edges[new_key] = {'directed': new_directed}
            self.redraw_graph()
            self.log(f"Тип зв'язку {u} та {v} змінено на: {'дуга' if new_directed else 'ребро'}")
            dialog.destroy()

        ttk.Button(dialog, text="Змінити тип зв'язку", command=toggle).pack(pady=6)
